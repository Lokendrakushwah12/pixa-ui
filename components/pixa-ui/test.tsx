...
const StackItem: React.FC<StackItemProps> = ({
  ...
  return (
    <motion.button
      onClick={onClick}
      className={`relative flex h-[300px] w-[300px] items-center justify-center overflow-visible rounded-lg border border-[var(--border)] shadow-lg`}
      transition={{ type: "spring", bounce: 0.3 }}
    >
      <motion.div
        className={`relative flex h-full w-full items-center justify-center ${isExpanded ? "z-[90]" : ""} `}
      ...
      <motion.span
        className="absolute bottom-0 w-full text-center text-base text-white"
...
const StackGrid: React.FC = () => {
  ...
  return (
    <MotionConfig reducedMotion="user">
      <div className="relative min-h-screen w-full max-w-6xl p-8">
        <AnimatePresence>
          {expandedIndex !== null && (
            <motion.div
